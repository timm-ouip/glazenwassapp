-- Reparatie op de vorige migratie, en de knop "Niet gewassen" scherper.
--
-- 1. `geldloop_wijziging_terugdraaien` was opnieuw aangemaakt vanaf een oude
--    versie, waardoor de reparaties uit geldloop_herstel en klant_nieuw_terug
--    weg waren (een nieuwe klant kwam niet terug bij zijn adres, een dossier
--    weigerde terug te draaien, en "laten stoppen" meldde succes zonder iets
--    terug te zetten). Hier staat de laatste versie, met de nieuwe tak erbij.
-- 2. De wasbeurt komt nu echt terug zoals hij was: met het stempel "gedaan"
--    (anders telt het geld niet mee) en met het eigen bedrag van die dag, dat
--    in een aparte rij staat.
-- 3. De knop pakt alleen een wasbeurt die al afgemeld is en niet in de
--    toekomst ligt. Anders zou een geldloper die in oktober de pof van
--    augustus ophaalt de geplande beurt van volgende week weghalen, zonder
--    dat er een cent van de pof af gaat.
-- 4. De dag moet de dag van zijn eigen avond zijn; alleen de eigenaar mag een
--    andere maand aanwijzen.
...
begin
  ...
  select wp.prijs into eigen_prijs from public.wasdag_prijzen wp where wp.regel_id = r.id;
  ...
  values (bedrijf, adres_id, vrij, 'niet_gewassen',
          to_jsonb(r) || jsonb_build_object('prijs', eigen_prijs), '{}'::jsonb, tekst, naam);
...
begin
  ...
  if w.soort = 'adres' then
    ...
    -- De hele regel is bewaard, dus hij komt terug zoals hij was. Het stempel
    -- "gedaan" mag alleen mee met de vlag erbij; zonder die vlag veegt de
    -- bewaker op wasdag_regels het weg en zou het geld niet terugkomen.
    ...
    -- De prijsregel maakt een trigger aan; daarna kan het eigen bedrag erin.
