...
export function SneltoetsenHulp({ open, onSluit }: { open: boolean; onSluit: () => void }) {
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onSluit()}>
      <PopupKader className="sm:max-w-sm" aria-describedby="sneltoetsen-uitleg">
        <DialogDescription id="sneltoetsen-uitleg" className="sr-only">
          Toetsen om sneller met je mail te werken
        </DialogDescription>
        <PopupKop icoon={<Keyboard className="size-5" />} titel="Sneltoetsen" subtitel="Werkt als je niet in een tekstvak typt" />
        <PopupBody className="gap-1.5">
          {SNELTOETSEN.map(([toets, wat]) => (
            <div key={toets} className="flex items-center justify-between gap-3 text-[13px]">
              <span>{wat}</span>
              <kbd className="rounded-md border border-border bg-muted px-1.5 py-0.5 font-mono text-[11.5px]">{toets}</kbd>
            </div>
          ))}
        </PopupBody>
      </PopupKader>
